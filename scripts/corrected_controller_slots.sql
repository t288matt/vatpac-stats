-- Corrected controller_slots vs flight_counts query
-- Handles frequencies_used elements that may be MHz strings (e.g., '132.200')
-- or already-Hz strings (e.g., '132200000'). Uses a ±5 kHz tolerance when joining to
-- transceivers to tolerate small frequency offsets from the VATSIM API.
...
controller_slots AS (
    SELECT
        ...
        END AS frequency,
        generate_series(
            GREATEST(cs.session_start_time, rm.monday_date + INTERVAL '8 hours'),
