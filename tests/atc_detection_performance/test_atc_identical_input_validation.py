...
class ATCTestingFramework:
    ...
    async def test_complete_matching_logic(self, flight_data: Dict[str, Any], atc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if existing_result and len(existing_result) > 0:
            try:
                existing_matches = int(existing_result[0]['raw_values'][0])
            except (IndexError, ValueError):
                logger.warning("Could not parse existing approach matches")
        
        if proposed_result and len(proposed_result) > 0:
            try:
                proposed_matches = int(proposed_result[0]['raw_values'][0])
            except (IndexError, ValueError):
                logger.warning("Could not parse proposed approach matches")
        ...
        matching_result = {
            ...
            'execution_time': execution_time,
            'data_accuracy': existing_matches == proposed_matches,
            'input_data': {
                'flight': flight_data,
                'atc': atc_data
            }
        ...
        logger.info(f"   Existing approach: {existing_matches:,} matches")
        logger.info(f"   Proposed approach: {proposed_matches:,} matches")
        logger.info(f"   Data accuracy: {'✅ PASS' if matching_result['data_accuracy'] else '❌ FAIL'}")
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 6: Validate input data consistency
            ...
            test_results = {
                ...
                'complete_matching': matching_result,
                ...
                'test_summary': {
                    ...
                    'data_accuracy': matching_result['data_accuracy'],
                    'success': input_consistent and functionally_equivalent and matching_result['data_accuracy']
                }
            }
