...
class EventBus:
    ...
    async def get_subscribers(self, event_type: str) -> List[Callable]:
    ...
    async def _handle_handler_failure(self, handler_name: str, error: Exception) -> None:
        ...
        # Log error with simplified handling
        self.logger.error(f"Handler execution failed for {handler_name}: {error}")
...
async def subscribe_to_event(event_type: str, handler: Callable) -> bool:
